<?php
	...
	class Boxalino_CemSearch_Helper_Data extends Mage_Core_Helper_Data {
		...
		public function isEnabled() {
			return (Mage::getStoreConfig('boxalinocemsearch/frontend/enabled') == 1);
		}

		public function isAnalyticsEnabled() {
			return (
				Mage::getStoreConfig('boxalinocemsearch/frontend/enabled') == 1 &&
				Mage::getStoreConfig('boxalinocemsearch/frontend/analytics') == 1
			);
		}
		...
		public function isSalesTrackingEnabled() {
			return (
				Mage::getStoreConfig('boxalinocemsearch/frontend/enabled') == 1 &&
				Mage::getStoreConfig('boxalinocemsearch/frontend/track_sales') == 1
			);
		}
		...
		public function isDebugEnabled() {
			return (Mage::getStoreConfig('boxalinocemsearch/frontend/debug') == 1);
		}


		public function getLanguage() {
			return Mage::getStoreConfig('boxalinocemsearch/service/language');
		}
		...
		public function isPageEnabled($uri) {
			if ($this->pagesEnabled == null) {
				...
				if ($this->isEnabled()) {
					foreach (explode(',', Mage::getStoreConfig('boxalinocemsearch/frontend/pageapi')) as $entry) {
						$entry = trim($entry);
						if (strlen($entry) > 0) {
		...
		public function loadPage($uri, $parameters = array()) {
			...
			if (!array_key_exists($uri, $this->pagesCache)) {
				$parameters['baseUri'] = $this->_getUrl('boxalinocemsearch/search');
		...
		public function getSuggestUrl() {
			...
			return $this->_getUrl('boxalinocemsearch/search/suggest');
		}
